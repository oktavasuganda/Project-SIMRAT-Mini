...
@section('content')
    <x-common.page-breadcrumb pageTitle="Edit Surat Keluar" />

    <div class="space-y-6">
        <div class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-800">
                <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
                    Edit: {{ $outbox->mail_number }}
                </h3>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <x-forms.input type="text" label="Nomor Agenda" name="agenda_number" required
                            :value="old('agenda_number', $outbox->agenda_number)" />
                    </div>
                    <div>
                        <x-forms.input type="text" label="Nomor Surat" name="mail_number" required :value="old('mail_number', $outbox->mail_number)" />
                    </div>

                    {{-- Baris 2: Tanggal Surat & Tujuan --}}
                    <div>
                        <x-forms.input type="text" label="Tanggal Surat" name="mail_date" id="mail_date" required
                            :value="old('mail_date', $outbox->mail_date)" />
                    </div>
                    <div>
                        <x-forms.input type="text" label="Ditujukan Kepada" name="intended_for" required
                            :value="old('intended_for', $outbox->intended_for)" />
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <div x-data="{ isOptionSelected: !!'{{ old('mail_category_id', $outbox->mail_category_id) }}' }">
                            <select name="mail_category_id" id="mail_category_id" required
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        ...
                                        {{ old('mail_category_id', $outbox->mail_category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    {{-- Baris 4: Subjek --}}
                    <div class="col-span-1 md:col-span-2">
                        <x-forms.input type="text" label="Subjek" name="subject" required :value="old('subject', $outbox->subject)" />
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <textarea name="summary" id="summary" placeholder="Masukkan ringkasan surat..." rows="6"
                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('summary', $outbox->summary) }}</textarea>
                        @error('summary')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        @if ($outbox->file_path)
                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">File saat ini: <a
                                    href="{{ Storage::url($outbox->file_path) }}" target="_blank"
