...
@section('content')
    <div class="container mx-auto p-4 bg-white dark:bg-gray-900 min-h-screen text-gray-900 dark:text-gray-100">
        <x-common.page-breadcrumb pageTitle="Detail Surat Keluar" />

        <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 border-b dark:border-gray-700 pb-4 mb-4">
                <h2 class="text-xl font-semibold md:col-span-2 text-gray-900 dark:text-gray-100">Informasi Surat Keluar</h2>
            </div>

            @php
                $filePath = $outbox->file_path;
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">{{ $outbox->subject }}</div>

                <div class="font-semibold text-gray-700 dark:text-gray-300">Ringkasan:</div>
                <div class="md:col-span-2">{{ $outbox->summary ?? '-' }}</div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 border-t dark:border-gray-700 pt-4 mt-4">
                        ...
                        @if ($previewable)
                            ...
                            <div
                                ...
                                @if (strtolower($extension) === 'pdf')
                                    <iframe src="{{ $fileUrl }}" class="w-full" style="height: 600px;" frameborder="0">
                ...
                <div>{{ \Carbon\Carbon::parse($outbox->created_at)->format('d M Y H:i:s') }}</div>
            </div>
